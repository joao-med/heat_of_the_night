...
for (i in city_names){
  ...
  # HNE
  HN <- crossbasis(data_city$HNE, lag = 10, 
...
#Coefs & (Co)v
coef_all_n %>%  write_rds(paste0("04.sensitivity//01.output//coef_all_04.rds"))
vcov_all_n %>% write_rds(paste0("04.sensitivity//01.output//vcov_all_04.rds"))
coef_all_CVD %>%  write_rds(paste0("04.sensitivity//01.output//coef_CVD_04.rds"))
vcov_all_CVD %>% write_rds(paste0("04.sensitivity//01.output//vcov_CVD_04.rds"))
coef_all_RD %>%  write_rds(paste0("04.sensitivity//01.output//coef_RD_04.rds"))
vcov_all_RD %>% write_rds(paste0("04.sensitivity//01.output//vcov_RD_04.rds"))
...
for(o in n_var){
  print(o)
  # loading coefs ------------------------------------------------------------
  coef_all <- read_rds(paste0("04.sensitivity//01.output//coef_",o,"_04.rds"))
  vcov_all <- read_rds(paste0("04.sensitivity//01.output//vcov_",o,"_04.rds"))
...
"
  plot_e <- P0+ p_1+p_2+p_3+p_4+p_5 +
    ...
    plot_annotation(title = paste0(e, " - tmean"))
  ggsave(paste0("04.sensitivity//03.plots//sa_",o,"_04.png"),
...
table_pred_complete <- bind_cols(table_pred_complete, sensitivity_analysis = "- Tmean")
# Filter only overall table
table_pred_complete <- table_pred_complete %>% filter (pred == "Overall")
# Saving table 
write.xlsx(table_pred_complete, paste0("04.sensitivity//02.tables//table_sa_04.xlsx"))
