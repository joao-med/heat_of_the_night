...
for (s in 1:2){
  ...
  for (i in city_names){
    ...
    cb_tmean <- crossbasis(data_city$mean, lag = lag,
                           arglag=list(fun="ns", knots = df[s]),
                           argvar=list(fun="ns", knots = df[s]),
                           group = data_city$year)
    # Relative humidity
    RH <- ns(data_city$rh, df = df[s])
    # hot nights index
...
"
    plot_e <- P0+ p_1+p_2+p_3+p_4+p_5 +
      ...
      plot_annotation(title = paste0(e, " tmean & RH df =",df[s]))
  ...
  table_pred_complete <- bind_cols(table_pred_complete, 
                                   sensitivity_analysis = paste0("tmean df =",df[s]))
