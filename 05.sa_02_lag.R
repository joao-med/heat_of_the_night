...
for (s in 1:2){
  ...
  for (i in city_names){
    ...
    # tmean
    cb_tmean <- crossbasis(data_city$mean, lag = lag[s],
    ...
    # HNE
    HN <- crossbasis(data_city$HNE, lag = lag[s], 
                     arglag = list(fun = "ns", knots = 3),
...
"
    plot_e <- P0+ p_1+p_2+p_3+p_4+p_5 +
      ...
      plot_annotation(title = paste0(e, ' lag = ',lag[s]))
  ...
  table_pred_complete <- bind_cols(table_pred_complete, 
                                   sensitivity_analysis = paste0("lag =",lag[s]))
